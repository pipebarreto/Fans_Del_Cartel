...
export default function AudiosMain(audios) {
    ...
    React.useEffect(() => {
      ...
      const PlayAudio = async () => {
          ...
          const result = await sounds.current.getStatusAsync();
          setAlong(result.positionMillis)
          //SetPlaying(false);
    ...
    async function player(audios) {
      ...
      setVisible(true);
      const {sound} = await sounds.current.loadAsync({uri:audios.link});
      //setSound(sound);
      setTrack(audios);
      await sounds.current.playAsync();
...
{/*}                <ListItem.Subtitle>{info}</ListItem.Subtitle>*/}
      ...
      {//    <Text/>
          ...
          buttonStyle={{borderRadius: 0, backgroundColor:'white'}}
           onPress={()=> sounds.current.setPositionAsync(along+300000)}/>
          </View>)}
